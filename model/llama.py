...
#
# This is a slightly modified version of the HuggingFace Llama implementation
# BASED ON TRANSFORMERS 4.34.0
#
...
try:
    ...
    print("Could not import is_flash_attn_available from transformers. Transformers version is outdated.")
    from transformers.utils import (
        add_start_docstrings,
        add_start_docstrings_to_model_forward,
        logging,
...
from quip.model.version import check_model_version
...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.upgate_proj = FusedQuipQuantLinear(
            ...
            codebook_name=config.quip_params.get('codebook', None),
        ...
        self.down_proj = QuipQuantLinear(
    ...
    def forward(self, x):
        ...
        output = self.down_proj(self.act_fn(gate_proj) * up_proj).half() #Cause issue.

        if output.isnan().any() or output.isinf().any():
            #breakpoint()
            print("OVERFLOW: MLP output is nan or inf")
            output = self.down_proj(self.act_fn(gate_proj) * up_proj)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.qkv_proj = FusedQuipQuantLinear(
            -1,
        ...
        self.o_proj = QuipQuantLinear(self.num_heads * self.head_dim,
                                      self.hidden_size,
                                      config.quip_params['codesz'],
                                      config.quip_params.get('packsz', 1),
                                      config.quip_params.get('pack_out', False),
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            #? hacky way to do it right now ?# 
            if query_states.isnan().any() or query_states.isinf().any(): 
                print("OVERFLOW: Query states is nan or inf")
                query_states, key_states, value_states = self.qkv_proj(hidden_states.to(torch.float32))
        ...
        if self.config.pretraining_tp > 1:
            ...
            if attn_output.isnan().any() or attn_output.isinf().any():
                #breakpoint()
                print("OVERFLOW: Attention output is nan or inf")
