...
class QuipQuantLinear(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight = self.Qidxs # ? Peft 0.5.0 Integration, not sure if it's being updated with Qidxs.

        self.codebook_name = codebook_name
        codebook_id_map = {"E8P12RVQ4B": 17, "E8P12": 7, "D4": 0}
        self.register_buffer("codebook_id", torch.tensor(codebook_id_map[self.codebook_name]))

        #* TODO: we need to adaptively select codebook *#
        #self.register_buffer("codebook_id", torch.tensor(7)) #* E8-2bit
        # self.register_buffer("codebook_id", torch.tensor(17)) #* E8-4bit
        # self.register_buffer("codebook_id", torch.tensor(0)) #* D4-4bit
